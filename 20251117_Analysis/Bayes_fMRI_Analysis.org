...
## *** The first duration is 10 seconds, not 15, fix this.
...
# in the contarst 'clench' vs. 'crosshair'. Create this design here.
## ** Backup the old design
design_bkp <- design

## ** Create contrasts
# We are going to contrast the clench and crosshair columns and store these in
# the design object. The code calculates a model for both positive and negative
# values, so no need to calculate two contrasts. In other words:
# 1) I create a column for Clench: the design convoluted with the HRF
# 2) I create a column for Rest (crosshair): the design convoluted with the HRF
# 3) I subtract these columns (this results in both positive + negative values)
# 4) Now you can estimate how well this signal predicts brain activation at each
#    region. If it predicts very well for all subjects, it will be significant.
#    - If it is a positive prediction: CL > rest.
#    - If it is a negative prediction: CL < rest.
contrasts <- c("CL_gt_CH")
design$design[, 1] <- (design_bkp$design[, 1] - design_bkp$design[, 2])
...
## ** Generate mean and thresholded images
...
:PROPERTIES:
:header-args:R:  :shebang "#!/usr/bin/env Rscript"
:header-args:R+: :tangle scripts/03_1st_level_loop.R
:header-args:R+: :mkdirp yes
:header-args:R+: :noweb yes
:END:
